...
const MEGA_PROMO_MIN_RECHARGE = 36
...
function calculateRecharge (amount, duration){
  ...
  if (amount > MIN_RECHARGE_AMOUNT){
    if (duration >= MEGA_PROMO_MIN_RECHARGE){
...
function main (){
  ...
  const duration = parseInt(prompt('Enter the duration of your subscription in months: '))

  const recharge = calculateRecharge (amount, duration)
  const promo = recharge - amount

  console.log(`
    THANKS FOR TRUST IN US - ${name}
        YOUR TIME WITH US: ${duration}
    --------------------------------
        RECHARGE: $${recharge}
        PROMO: $${promo}
    `)
